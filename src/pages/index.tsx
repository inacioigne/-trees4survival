...
export default function Home({main, items}:Props) {
  ...
  return (
    <div>
        <Header title="Trees for Survival" sections={sections} />
        <Divider />
        <main>
            <MainPost item={main} />
...
async function GetData(obj:any) {
  ...
  const item:Item = await api.get(`items/${obj.UUID}/metadata`, {
    headers: {
...
export async function getStaticProps() {
  ...
  const item:Item = await api.get('items/30e17215-d854-4e31-98fd-f7931cb0c735/metadata', {
        headers: {
          'Access-Control-Allow-Origin': '*',
        },
      })
      ...
      const listItems = [ 
        {UUID: '8656c9a0-96b3-4b57-bf63-1f64224a5956', img: "/image/pupunheira.jfif"},
      {UUID: "b2cccf80-95b6-451f-9eef-66b3e7ea70d2", img: "/image/pupunheira.jfif"}
        ]
